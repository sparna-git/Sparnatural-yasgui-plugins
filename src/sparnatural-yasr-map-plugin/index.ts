...
export class MapPlugin implements SparnaturalPlugin<PluginConfig> {
  ...
  public static defaults: PluginConfig = {
    ...
    L18n: {
      warnFindNoCoordinate:
        "Attention, des résultats (<count>) n'ont pas de coordonnées pour être représentés sur la carte.",
  ...
  constructor(yasr: Yasr) {
    ...
    if (MapPlugin.defaults.markerOptions)
      L.Marker.mergeOptions(MapPlugin.defaults.markerOptions);
    if (MapPlugin.defaults.polylineOptions)
      L.Marker.mergeOptions(MapPlugin.defaults.polylineOptions);
  ...
  canHandleResults(): boolean {
    ...
    if (tableXResults && tableXResults.getBindings().length > 0) {
      tableXResults.getBindings().some((row: Parser.Binding) => {
            ...
            for (var anotherKey in row) {
              ...
              if (longKey) {
                ...
                geoLiterals.push({
                  cellValue: {
                    value:
                      "POINT(" + row[longKey].value + " " + cell.value + ")",
                    type: "literal",
  ...
  draw(persistentConfig: any, runtimeConfig?: any): void | Promise<void> {
    ...
    this.results.getBindings().forEach((row: Parser.Binding) => {
      ...
      return geoCells.map((geoCell) => {
        ...
        if (geoCell.parsedLit?.type === "Point")
          this.drawMarker(geoCell.parsedLit, geoCell.variable, popUpString);
    ...
    this.controlLayers = L.control.layers({}, controlsLayersObjs, {
      collapsed: false,
    });
  ...
  private drawPoly(
    ...
    if (this.colorsUsed.includes(colIndex)) {
      polyOptions["color"] =
        this.config.polygonColors[this.colorsUsed.indexOf(colIndex)];
    } else {
