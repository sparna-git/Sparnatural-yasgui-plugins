...
export class TableXResults implements Parser {
  ...
  constructor(
    parser: Parser,
    bindingSetAdapter?: (bindingSet: Parser.Binding) => Parser.Binding
  ) {
    this.bindings = parser.getBindings();
    // process complete binding sets
    if (bindingSetAdapter) {
      this.bindings = this.bindings.map((bindingSet) =>
        bindingSetAdapter(bindingSet)
      );
    }
    // add new bindings with labels + uris
    this.bindings = this.bindings.map((bindingSet) =>
      this.enhanceBinding(bindingSet)
    );
    // remove _label from list of variables
    this.variables = parser
      .getVariables()
      .filter(
        (variable) =>
          !(
            ...
            parser
              .getVariables()
              .includes(
                variable.substring(0, variable.length - "_label".length)
              )
          )
      );
  }
  ...
  /*
    private enhanceBindingDate(bindingSet: Parser.Binding): Parser.Binding {
    var newBinding = {};
    for (var key in bindingSet) {
      // if we find the same key woth _label in the binding set
      // if(key+"_label" in bindingSet) {
      if (
        bindingSet[key + "_begin"] != undefined &&
        bindingSet[key + "_end"] != undefined
      ) {
        // then recreate a special binding in the binding set with the URI and the label
        var begin = bindingSet[key + "_begin"].value;
        var end = bindingSet[key + "_end"].value;
        newBinding[key] = {
          value: bindingSet[key].value,
          type: "x-dated-uri",
          datatype: bindingSet[key].datatype,
        ...
        console.log("newBinding[key] :", newBinding[key]);
      } else if (
        // if the key ends with xxx_begin and the key xxx exists in the binding set, then ignore it
        key.endsWith("_begin") ||
        key.endsWith("_end") ||
        (key.substring(0, key.length - "_begin".length) in bindingSet &&
          key.substring(0, key.length - "_end".length) in bindingSet)
      ) {
        // don't include it in bindings
      } else {
        // otherwise just include as normal
